...
+ (ZipArchive *) process:(ZipArchive *) archiveFile withItems:(id)item usingResourceFork:(BOOL)rfFlag withInvoker:(id)invoker andDelegate:(id)delegate {
    

    //检查archiveFile类型
//    if([archiveFile isKindOfClass:[ZipArchive class]] || !archiveFile){
//        return nil;
//    }
//    NSLog(@"type : %c",[archiveFile isKindOfClass:[ZipArchive class]]);	
...
//			archiveFile = [self archiveWithArchivePath:path];
//			if (!archiveFile)
//				return nil;
			archiveFile.invoker = invoker;
			if (delegate) {
				archiveFile.delegate = delegate;
				if ([archiveFile delegateWantsSizes]) {
					if ([NSThread isMainThread]) {
						[archiveFile didUpdateTotalSize:[archiveFile.centralDirectory valueForKeyPath:@"@sum.uncompressedSize"]];
						[archiveFile didUpdateTotalCount:[NSNumber numberWithUnsignedLongLong:[archiveFile.centralDirectory count]]];
					} else {
						[archiveFile performSelectorOnMainThread:@selector(didUpdateTotalSize:)
						                          withObject:[archiveFile.centralDirectory valueForKeyPath:@"@sum.uncompressedSize"]
						                       waitUntilDone:NO];
						[archiveFile performSelectorOnMainThread:@selector(didUpdateTotalCount:)
						                          withObject:[NSNumber numberWithUnsignedLongLong:[archiveFile.centralDirectory count]]
						                       waitUntilDone:NO];
					}
				}
				if ([NSThread isMainThread])
					[archiveFile didBeginUnzip];
				else
					[archiveFile performSelectorOnMainThread:@selector(didBeginUnzip) withObject:nil waitUntilDone:NO];
			}
			
			NSInteger result = [archiveFile inflateToDiskUsingResourceFork:rfFlag];
			if (result == zkSucceeded) {
				if (archiveFile.delegate) {
					if ([NSThread isMainThread])
						[archiveFile didEndUnzip];
					else
						[archiveFile performSelectorOnMainThread:@selector(didEndUnzip) withObject:nil waitUntilDone:NO];
				}
			} else if (result == zkCancelled) {
				if (archiveFile.delegate) {
					if ([NSThread isMainThread])
						[archiveFile didCancel];
					else
						[archiveFile performSelectorOnMainThread:@selector(didCancel) withObject:nil waitUntilDone:NO];
				}
				...
				if (archiveFile.delegate) {
					if ([NSThread isMainThread])
						...
						[archiveFile performSelectorOnMainThread:@selector(didFail) withObject:nil waitUntilDone:NO];
				}
			}
		} else {
...
//			archiveFile = [self archiveWithArchivePath:archivePath];
//			if (!archiveFile)
//				return nil;
			archiveFile.invoker = invoker;
			if (delegate) {
				archiveFile.delegate = delegate;
				[NSThread detachNewThreadSelector:@selector(calculateSizeAndItemCount:) toTarget:archiveFile
				                       withObject:@{ZKPathsKey: @[path],
						   ZKusingResourceForkKey: @(rfFlag)}];
				if ([NSThread isMainThread])
					[archiveFile didBeginZip];
				else
					[archiveFile performSelectorOnMainThread:@selector(didBeginZip) withObject:nil waitUntilDone:NO];
			}
			NSInteger result = zkSucceeded;
			if ([archiveFile.fileManager zk_isDirAtPath:path] && ![archiveFile.fileManager zk_isSymLinkAtPath:path])
				result = [archiveFile deflateDirectory:path relativeToPath:[path stringByDeletingLastPathComponent] usingResourceFork:rfFlag];
			else
				result = [archiveFile deflateFile:path relativeToPath:[path stringByDeletingLastPathComponent] usingResourceFork:rfFlag];
			if (result == zkSucceeded) {
				if (archiveFile.delegate) {
					if ([NSThread isMainThread])
						[archiveFile didEndZip];
					else
						[archiveFile performSelectorOnMainThread:@selector(didEndZip) withObject:nil waitUntilDone:NO];
				}
			} else if (result == zkCancelled) {
				[archiveFile.fileManager removeItemAtPath:archivePath error:nil];
				if (archiveFile.delegate)
					[archiveFile performSelectorOnMainThread:@selector(didCancel) withObject:nil waitUntilDone:NO];
			} else if (result == zkFailed) {
				[archiveFile.fileManager removeItemAtPath:archivePath error:nil];
...
//			return nil;
		...
		NSInteger result = [archiveFile deflateFiles:paths relativeToPath:basePath usingResourceFork:rfFlag];
		if (result == zkSucceeded) {
			if (archiveFile.delegate) {
				if ([NSThread isMainThread])
					[archiveFile didEndZip];
...
+ (ZipArchive *) archiveWithArchivePath:(NSString *)path {
